...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      supabase.from('orders').select('id, total, status, created_at, profiles(full_name)', { count: 'exact' }).order('created_at', { ascending: false }).limit(5),
      supabase.from('profiles').select('id', { count: 'exact', head: true }),
      supabase.from('orders').select('total'),
    ]).then(([products, orders, users, allOrders]) => {
      const revenue = (allOrders.data || []).reduce((sum, o) => sum + Number(o.total), 0);
      setStats({
      ...
      if (orders.data) setRecentOrders(orders.data as Order[]);
      setLoading(false);
    });
  }, []);

  const statCards = [
    { label: 'Productos', value: stats.products, icon: Package, color: 'bg-sky-50 text-sky-600', change: '+3 este mes' },
    { label: 'Pedidos', value: stats.orders, icon: ShoppingBag, color: 'bg-orange-50 text-orange-600', change: 'Total histórico' },
    { label: 'Clientes', value: stats.users, icon: Users, color: 'bg-green-50 text-green-600', change: 'Registrados' },
    { label: 'Ingresos', value: `$${stats.revenue.toFixed(2)}`, icon: DollarSign, color: 'bg-amber-50 text-amber-600', change: 'Total vendido' },
  ];
